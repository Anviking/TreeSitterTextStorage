...
public func tokenize(string: String, tokenize: Symbol -> UIColor?) -> NSAttributedString {
    ...
    func traverseNode(node: TSNode, depth: Int = 0) {
        for i in 0 ..< ts_node_child_count(node) {
            ...
            print(string)
            let symbol = Symbol(rawValue: ts_node_symbol(child))!
            print(symbol)
...
public enum Symbol: UInt16, CategorizableTokenType {
    ...
    var tokenType: TokenType {
        ...
        case .sym_number_literal:
            return .Number
        case .anon_sym_return, .anon_sym_while, .anon_sym_static, .anon_sym_const, .anon_sym_volatile, sym_function_specifier, .anon_sym_if, .anon_sym_else:
            return .Keyword
        case .sym_type_name:
